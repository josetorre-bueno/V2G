...
// ── Uncoordinated (Green Button) mode ─────────────────────────────────────────
function parseGreenButtonCSV(text, month) {
  // Strip BOM if present
  const clean = text.replace(/^\uFEFF/, '');
  const lines = clean.split(/\r?\n/);

  // Find the header line containing DATE, START TIME, USAGE
  let headerIdx = -1;
  for (let i = 0; i < lines.length; i++) {
    const upper = lines[i].toUpperCase();
    if (upper.includes('DATE') && upper.includes('START TIME') && upper.includes('USAGE')) {
      headerIdx = i;
      break;
    }
  }
  ...
  if (headerIdx < 0) {
    throw new Error('Green Button CSV format not recognized — header line with DATE not found');
  }

  // Accumulate kWh by (dateStr + '|' + hour) and track unique dates per hour
  const hourlyMap  = {};   // key -> total kWh
  const hourlyDays = {};   // hour -> Set of date strings
  ...
  for (let i = headerIdx + 1; i < lines.length; i++) {
    ...
    const kwh       = parseFloat(parts[3]);

    if (isNaN(kwh) || parts[3].trim() === '') continue;
    ...
    // Parse hour from start time HH:MM
    const hour = parseInt(startTime.split(':')[0]);
    if (isNaN(hour)) continue;

    const key = dateStr + '|' + hour;
    hourlyMap[key] = (hourlyMap[key] || 0) + kwh;

    if (!hourlyDays[hour]) hourlyDays[hour] = new Set();
    hourlyDays[hour].add(dateStr);
  }

  // Collect all unique weekday dates that contributed any data
  const allDates = new Set();
  for (const key of Object.keys(hourlyMap)) {
    allDates.add(key.split('|')[0]);
  }
  const dayCount = allDates.size;

  if (dayCount === 0) {
    throw new Error(`No weekday data found for month ${month} in this Green Button file`);
  }

  // Average over contributing days for each of 24 hours
  const avgHourlyLoad = Array(24).fill(0);
  for (let h = 0; h < 24; h++) {
    const days = hourlyDays[h];
    if (!days || days.size === 0) { avgHourlyLoad[h] = 0; continue; }
    let total = 0;
    for (const d of days) {
      total += hourlyMap[d + '|' + h] || 0;
    }
    avgHourlyLoad[h] = total / days.size;
  }

  return { avgHourlyLoad, dayCount };
}
...
function buildSummaryCSV(state, stdRes, optRes, uncoordRes) {
  ...
  const s = stdRes, o = optRes, u = uncoordRes || null;
  const rows = [];
  rows.push('\uFEFF');
  ...
  rows.push('');
  rows.push('RESULTS');
  rows.push(`Metric,Standard,Optimized,Uncoordinated`);
  rows.push(`V2G Penetration,${(s.penetration*100).toFixed(3)}%,${(o.penetration*100).toFixed(3)}%,${u ? (u.penetration*100).toFixed(3)+'%' : 'N/A'}`);
  rows.push(`Vehicle Count,${s.vehicles},${o.vehicles},${u ? u.vehicles : 'N/A'}`);
  rows.push(`Max Battery Draw (kWh),${s.max_draw_kwh},${o.max_draw_kwh},${u ? u.max_draw_kwh : 'N/A'}`);
  rows.push(`Max Draw (% of Battery),${(s.max_draw_pct*100).toFixed(1)}%,${(o.max_draw_pct*100).toFixed(1)}%,${u ? (u.max_draw_pct*100).toFixed(1)+'%' : 'N/A'}`);
  rows.push(`Grid Energy Needed (kWh),${s.grid_energy},${o.grid_energy},${u ? u.grid_energy : 'N/A'}`);
  rows.push(`Battery Energy Delivered (kWh),${s.batt_delivered},${o.batt_delivered},${u ? u.batt_delivered : 'N/A'}`);
  rows.push('');
...
const MANUAL = [
  {
    heading: "OVERVIEW",
    body: "The V2G Analysis Engine calculates the minimum percentage of EVs on a distribution circuit that must be V2G-capable to shave the circuit's peak load below a set threshold. Three models are available: Standard, Optimized, and Uncoordinated. All computation runs entirely in the browser — no data is sent to any server.",
  },
  {
    heading: "THE THREE MODELS",
    bullets: [
      "Standard — A simplified analytical model. All V2G cars discharge at whatever rate is needed to hold the circuit exactly at the threshold during every peak hour. No coordination is assumed between cohorts. Gives a conservative (higher) vehicle count because each car must independently cover the peak in every hour it is home.",
  ...
  },
  {
    ...
    bullets: [
      ...
      "Circuit Month — month (1\u201312) of the base load data. Used to filter the Green Button file to the correct seasonal average for the Uncoordinated model.",
    ],
  },
  {
    heading: "HOUSEHOLD LOAD PROFILE (GREEN BUTTON)",
    body: "Used only by the Uncoordinated model. Upload a Green Button CSV file (SDG&E My Energy Center export format). The app parses the file and computes the average weekday kWh per hour for the selected Circuit Month. The resulting 24-hour profile represents a typical household's hourly consumption — the amount each V2G car will attempt to offset from the grid.",
    body2: "Both 15-minute and hourly Green Button files are accepted. Weekends are excluded from the average. The derived profile is shown in the read-only table and is saved in the Summary CSV for future restores.",
  },
  {
    heading: "RUNNING THE ANALYSIS",
    body: "Load a Circuit CSV to run automatically, or fill in the circuit fields manually and click \u25b6 Run Analysis. The Standard and Optimized models always run. The Uncoordinated model runs only when household load data has been loaded from a Green Button file. Results appear immediately — all calculation is done in the browser.",
  },
  {
    ...
    bullets: [
      ...
      "Max Draw (kWh) — maximum cumulative kWh withdrawn from a single car's battery across the entire peak or discharge window. Standard: sum of per-hour battery draws. Optimized: the optimal per-car energy target T. Uncoordinated: total draw for the earliest-arriving cohort.",
      ...
      "Battery Energy Delivered (kWh) — total kWh delivered to the grid by the entire V2G fleet. In Optimized mode this should equal Grid Energy Needed (the energy checksum). In Standard mode the values may differ slightly due to cohort boundary effects. In Uncoordinated mode they will generally differ because cars are responding to house load, not circuit deficit.",
    ],
  },
  {
    heading: "ENERGY CHECKSUM",
    body: "The green \u2713 Energy checksum balanced message confirms that the Optimized model's total battery delivery exactly equals the grid deficit — a mathematical validation that the dispatch algorithm is internally consistent. A mismatch indicates a calculation error. The Standard and Uncoordinated models do not carry this guarantee by design.",
  },
  {
    heading: "OUTPUT FILES",
    bullets: [
      "Circuit_N_OS.csv — Standard mode output. 37-row \u00d7 13-column table matching the layout of the original Excel model (V2GModel.xlsx). Columns: Hour, Base Load, Future Year Change, % Cars Home, Load Increase, BEV Availables, Energy to Discharge, Projected kW, Managed kW, Threshold, Capacity. Parameter summary in rows 1\u201311.",
      "Circuit_N_OO.csv — Optimized mode output. Same structure as OS. The Managed kW column shows the load after optimised V2G dispatch — it should sit at or just below the threshold during all peak hours.",
      ...
      "Circuit_N_HistoryS.csv — Standard mode cohort history. One row per hour, one pair of columns per arrival cohort (C1\u2013C24). _Draw_kWh = cumulative per-car battery draw up to that hour. _Ratio = per-hour draw as a fraction of EVSE capacity. Header row shows GridNeeded and BattDelivered totals.",
      "Circuit_N_HistoryO.csv — Optimized mode cohort history. Same structure. GridNeeded and BattDelivered in the header should be equal (energy checksum).",
...
function App() {
  ...
  const [circuitMonth,         setCircuitMonth]         = useState('7');
  const [houseLoad,            setHouseLoad]            = useState(Array(24).fill(''));
  const [greenButtonRaw,       setGreenButtonRaw]       = useState('');
  const [greenButtonMsg,       setGreenButtonMsg]       = useState(null);
  const greenButtonInputRef = useRef(null);
  ...
  // Month change handler — reparse green button data if available
  const handleMonthChange = useCallback(val => {
    setCircuitMonth(val);
    if (greenButtonRaw) {
      try {
        const mo = Math.max(1, Math.min(12, parseInt(val) || 7));
        const { avgHourlyLoad, dayCount } = parseGreenButtonCSV(greenButtonRaw, mo);
        setHouseLoad(avgHourlyLoad.map(v => v.toFixed(3)));
        setGreenButtonMsg({ ok: true, text: `Reparsed: ${dayCount} weekdays in month ${mo}` });
      } catch (e) {
        setGreenButtonMsg({ ok: false, text: e.message });
      }
    }
  }, [greenButtonRaw]);

  // Green Button CSV file handler
  const handleGreenButtonFile = useCallback(e => {
    ...
    reader.onload = ev => {
      ...
      try {
        const mo = Math.max(1, Math.min(12, parseInt(circuitMonth) || 7));
        const { avgHourlyLoad, dayCount } = parseGreenButtonCSV(text, mo);
        setGreenButtonRaw(text);
        setHouseLoad(avgHourlyLoad.map(v => v.toFixed(3)));
        setGreenButtonMsg({ ok: true, text: `${dayCount} weekdays in month ${mo} — ${file.name}` });
      } catch (err) {
        setGreenButtonMsg({ ok: false, text: `Parse failed: ${err.message}` });
      }
    };
  ...
  const handleRun = useCallback((overrides = {}) => {
    ...
    try {
      ...
      setResults({ standard: stdRes, optimized: optRes, uncoordinated: uncRes, params });
      setStatus('done');
    } catch (e) {
    ...
    <div style={{ background: C.bg, minHeight: '100vh', color: C.text, fontFamily: "'DM Sans','DM Mono',sans-serif", padding: '24px 20px' }}>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto 22px' }}>
        ...
        <p style={{ fontSize: 12, color: C.muted, lineHeight: 1.6 }}>
          ...
          Load a circuit CSV to run Standard, Optimized, and Uncoordinated analysis automatically.
        </p>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Card title="Household Load Profile (Green Button)">
            <div style={{ fontSize: 11, color: C.muted, marginBottom: 8, lineHeight: 1.5 }}>
              Average weekday kWh per hour for the circuit month (set in Circuit Parameters).
              Used by the Uncoordinated model.
            </div>
            <div style={{ display: 'flex', gap: 8, marginBottom: 8, alignItems: 'center' }}>
              <input type="file" accept=".csv" ref={greenButtonInputRef} style={{ display: 'none' }} onChange={handleGreenButtonFile}/>
              <Btn color="muted" onClick={() => greenButtonInputRef.current && greenButtonInputRef.current.click()}>
                📂 Load Green Button CSV
              </Btn>
            </div>
            {greenButtonMsg && (
              <div style={{ fontSize: 11, color: greenButtonMsg.ok ? C.green : C.red, marginBottom: 8 }}>
                {greenButtonMsg.text}
              </div>
            )}
            <HourlyTable values={houseLoad.length === 24 ? houseLoad : Array(24).fill('')} onChange={null} />
            <div style={{ fontSize: 10, color: C.faint, marginTop: 4 }}>kWh/hr average — read-only, derived from Green Button file</div>
          </Card>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <Card title="Circuit Parameters">
            ...
            <Field label="Circuit Month"    value={circuitMonth}    onChange={handleMonthChange}  note="1–12 (filters Green Button data)"/>
          </Card>
          ...
          {/* Results */}
          {results && (() => {
            const { standard: s, optimized: o, uncoordinated: u, params } = results;
            ...
            return (
              <Card title="Results">
                <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr 1fr 1fr', gap: 6, marginBottom: 12 }}>
                  {['', 'Standard', 'Optimized', 'Uncoordinated'].map((h, i) => (
                    <div key={i} style={{ fontSize: 10, color: i === 0 ? C.faint : i === 3 ? C.orange : C.accent, fontFamily: "'DM Mono',monospace", letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 600, paddingBottom: 4, borderBottom: `1px solid ${C.border}` }}>{h}</div>
                  ))}
                  {[
                    ['Penetration', `${(s.penetration*100).toFixed(3)}%`, `${(o.penetration*100).toFixed(3)}%`, u ? `${(u.penetration*100).toFixed(3)}%${u.feasible ? '' : ' ⚠'}` : '—'],
                    ['Vehicles',    s.vehicles,  o.vehicles,  u ? u.vehicles : '—'],
                    ['Max Draw',    `${s.max_draw_kwh} kWh`, `${o.max_draw_kwh} kWh`, u ? `${u.max_draw_kwh} kWh` : '—'],
                    ['Max Draw %',  `${(s.max_draw_pct*100).toFixed(1)}%`, `${(o.max_draw_pct*100).toFixed(1)}%`, u ? `${(u.max_draw_pct*100).toFixed(1)}%` : '—'],
                    ['Grid Energy', `${s.grid_energy} kWh`, `${o.grid_energy} kWh`, u ? `${u.grid_energy} kWh` : '—'],
                    ['Batt Delivered', `${s.batt_delivered} kWh`, `${o.batt_delivered} kWh`, u ? `${u.batt_delivered} kWh` : '—'],
                  ].map(([label, sv, ov, uv]) => (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  <Btn full color="orange" onClick={() => downloadCSV(buildSummaryCSV(uiState(), s, o, u), `Circuit_${circuitNum}_V2G_summary.csv`)}>
